...
export function renderHistogram(activity7d = [], maxDaily = 1) {
  ...
  return `
    <div class="histogram-container space-y-4" data-histogram>
      ...
      <div class="grid grid-cols-7 gap-2 sm:gap-3 items-end h-56 pt-8 pb-3 px-3 bg-surface-subtle rounded-2xl border border-outline-variant relative overflow-hidden">
        <!-- Scale Grid Lines -->
        <div class="absolute inset-x-0 top-6 border-b border-dashed border-outline-variant/70 pointer-events-none"></div>
        <div class="absolute inset-x-0 top-1/2 border-b border-dashed border-outline-variant/70 pointer-events-none"></div>
        ...
        ${velocity.days.map((d) => {
          ...
          return `
            <div class="flex flex-col items-center h-full justify-end group relative z-10">
              <!-- Hover Tooltip -->
              <div class="opacity-0 group-hover:opacity-100 transition-opacity absolute -top-8 bg-stone-accent text-white px-2 py-0.5 rounded-md text-[10px] font-mono whitespace-nowrap z-20 pointer-events-none shadow-lg">
                ${d.dateStr}: ${d.count} tasks (+${d.xp} XP)
              </div>

              <!-- Bar Stat Label -->
              <span class="text-[10px] font-mono text-outline group-hover:text-stone-accent transition-colors mb-1">
                ${d.count > 0 ? `+${d.xp}` : "0"}
              </span>

              <!-- Histogram Column Bar -->
              <div class="w-full max-w-[32px] sm:max-w-[48px] rounded-lg flex flex-col justify-end overflow-hidden transition-all duration-300" style="height: ${d.pctHeight}%;">
                <div class="w-full h-full ${hasActivity ? (isToday ? 'bg-gradient-to-t from-blue-700 to-indigo-500 shadow-md shadow-blue-500/25' : 'bg-primary/80 group-hover:bg-primary') : 'bg-surface-container-high'} transition-all"></div>
              </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 font-mono text-xs">
        <div class="p-3 bg-surface rounded-2xl border border-outline-variant shadow-card flex items-center justify-between">
          <span class="text-secondary">7-DAY ACTIVITY:</span>
          <span class="font-bold text-stone-accent">${velocity.totalCount} TASKS</span>
        </div>
        <div class="p-3 bg-surface rounded-2xl border border-outline-variant shadow-card flex items-center justify-between">
          <span class="text-secondary">XP CYCLE VELOCITY:</span>
          <span class="font-bold text-primary">+${velocity.totalXp} XP</span>
        </div>
        <div class="p-3 bg-surface rounded-2xl border border-outline-variant shadow-card flex items-center justify-between">
          <span class="text-secondary">DAILY AVERAGE:</span>
          <span class="font-bold text-success">~${velocity.avgXp} XP / DAY</span>
        </div>
      </div>
    </div>
  `;
...
export function renderTelemetryView(container) {
  ...
  container.innerHTML = `
    <div class="max-w-6xl mx-auto space-y-8" data-telemetry-view>
      ...
      <section class="p-6 bg-surface rounded-3xl border border-outline-variant shadow-card space-y-6">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-outline-variant pb-6">
          <div class="space-y-2">
            <div class="flex items-center gap-2">
              ...
              <span class="font-sans text-xs font-bold tracking-widest text-primary font-mono">STATS &amp; HISTORY</span>
              <span class="text-outline-variant">/</span>
              <span class="font-mono text-xs text-outline">CLASS_PROGRESSION</span>
            </div>
            <div class="flex items-baseline gap-4 flex-wrap">
              <span class="font-mono text-xs px-2.5 py-1 rounded-lg bg-primary-soft border border-blue-200 text-primary font-bold">
                LEVEL ${String(currentLevel).padStart(2, "0")}
              </span>
              <h1 class="font-sans text-3xl font-extrabold tracking-tight text-stone-accent uppercase">
            ...
            <p class="text-xs text-secondary">
              Immutable historical logs for completions, productivity velocity, and streak analysis.
            </p>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-5 gap-2">
            ${RPG_TIERS.map((tier, idx) => {
              ...
              const borderClass = isActive
                ? "border-primary bg-primary-soft text-stone-accent shadow-sm"
                : isPast
                ? "border-outline-variant bg-surface-subtle text-stone-soft"
                : "border-outline-variant/60 bg-white text-outline opacity-70";

              return `
                <div class="p-3 rounded-xl border ${borderClass} font-mono text-xs flex flex-col justify-between space-y-2 transition-colors">
                  ...
                  <div>
                    <h4 class="font-sans font-bold text-xs uppercase tracking-wider ${isActive ? 'text-primary' : isPast ? 'text-stone-accent' : 'text-outline'}">
                      ${tier.name}
                    </h4>
                    <p class="text-[10px] text-secondary line-clamp-2 mt-1 font-sans">
                      ${tier.description}
                    </p>
                  </div>
                </div>
        ...
        <div class="space-y-2 pt-2 border-t border-outline-variant/70">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section class="space-y-4">
          <div class="flex items-center justify-between border-b border-outline-variant pb-2">
            <span class="font-sans text-sm font-extrabold text-stone-accent">CATEGORY COMPLETION METERS</span>
            <span class="font-mono text-xs text-outline">QUOTA DISTRIBUTION</span>
          </div>

          <div class="p-6 bg-surface rounded-3xl border border-outline-variant shadow-card space-y-4 font-mono text-xs">
            ${categoriesBreakdown.map((cat) => {
              return `
                <div class="space-y-1.5">
                  <div class="flex items-center justify-between">
                    <span class="font-bold text-stone-accent uppercase">${escapeHtml(cat.category)}</span>
        ...
        <section class="space-y-4">
          <div class="flex items-center justify-between border-b border-outline-variant pb-2">
            <span class="font-sans text-sm font-extrabold text-stone-accent">WALLET & DISCIPLINE AUDIT</span>
            <span class="font-mono text-xs text-outline">METRICS SNAPSHOT</span>
          </div>

          <div class="p-6 bg-surface rounded-3xl border border-outline-variant shadow-card space-y-6 font-mono text-xs">
            <!-- Coin Wallet Telemetry -->
            <div class="space-y-3">
              <span class="text-outline block text-[11px] uppercase">COIN ECONOMY STATUS</span>
              <div class="grid grid-cols-3 gap-3 text-center">
                ...
                <div class="p-3 bg-success-soft rounded-xl border border-emerald-200">
                  ...
                  <span class="font-bold text-success text-sm mt-1 block">+${lifetimeEarned}</span>
                </div>
                <div class="p-3 bg-primary-soft rounded-xl border border-blue-200">
                  <span class="block text-[10px] text-primary">LIFETIME SPENT</span>
                  <span class="font-bold text-primary text-sm mt-1 block">-${lifetimeSpent}</span>
                </div>
              </div>
            </div>

            <!-- Productivity Health Readout -->
            <div class="space-y-3 pt-4 border-t border-outline-variant/70">
              <span class="text-outline block text-[11px] uppercase">FOCUS ENGINE ACCRUAL</span>
              <div class="grid grid-cols-2 gap-3">
                <div class="p-3 bg-surface-subtle rounded-xl border border-outline-variant">
                  <span class="block text-[10px] text-outline">TOTAL FOCUS ACCRUED</span>
                  <span class="font-bold text-stone-accent text-sm mt-1 block">${focusDisplay}</span>
                ...
                <div class="p-3 bg-surface-subtle rounded-xl border border-outline-variant">
                  ...
                  <span class="font-bold text-stone-accent text-sm mt-1 block">${doneTasks} / ${totalTasks}</span>
