#!/bin/bash
# dtask-web supervisor — restart bun if dead
# Registered as a no-agent Hermes cron (every 15m) so dtask-web auto-heals
# after VPS restarts or accidental kills.
#
# Logic:
#  1. Skip if bun just started <2 min ago (avoid restart churn)
#  2. Health = dtask-web's bun is alive AND /api/health returns {"ok":true}
...
#
# Logs:
#  /tmp/dtask-supervisor.log  (only when action taken)
#  /tmp/dtask.log             (bun stdout — always append)

set -e
PORT="${DTASK_PORT:-${BTASK_PORT:-8787}}"
DIR="${DTASK_DIR:-${BTASK_DIR:-/opt/data/repos/dtask-web}}"
DB="${DTASK_DB:-${BTASK_DB:-$DIR/db.sqlite}}"
...
if find_dtask_bun >/dev/null 2>&1; then
...
echo "[dtask-supervisor] $(date -u) bun unhealthy, restarting" >> /tmp/dtask-supervisor.log
graceful_kill
sleep 1
checkpoint_wal
cd "$DIR"
nohup /opt/data/.bun/bin/bun server.ts >> /tmp/dtask.log 2>&1 &
disown
sleep 2
echo "[dtask-supervisor] restarted pid=$(find_dtask_bun 2>/dev/null)" >> /tmp/dtask-supervisor.log
